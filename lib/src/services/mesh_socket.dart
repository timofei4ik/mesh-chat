...
class MeshSocket {
  ...
  Future<void> connect({
    ...
    channel.sink.add(jsonEncode(_helloPacket(session, publicKey)));
    ...
    _subscription = channel.stream.listen(
      ...
      onError: (Object error) {
        ...
        _scheduleReconnect(session, publicKey, onPacket, onStatus);
      },
      onDone: () {
        _connected = false;
        onStatus('Offline');
  ...
  Future<ConnectionDiagnostics> diagnose(
    ...
    try {
      ...
      channel.sink.add(
        jsonEncode({
          ..._helloPacket(session, publicKey),
  ...
  Map<String, dynamic> _helloPacket(Session session, String publicKey) {
    return {
      'type': 'server_hello',
