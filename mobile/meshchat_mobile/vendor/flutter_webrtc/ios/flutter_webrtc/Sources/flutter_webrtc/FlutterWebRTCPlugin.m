...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
  if ([@"meshNoiseConfigure" isEqualToString:call.method]) {
    BOOL enabled = [call.arguments[@"enabled"] boolValue];
    AudioProcessingAdapter* adapter = _audioManager.capturePostProcessingAdapter;
    if (enabled && !_meshNoise && adapter) {
      _meshNoise = [[MeshNoiseProcessor alloc] init];
      [adapter addProcessing:_meshNoise];
    } else if (!enabled && _meshNoise) {
  ...
  if ([@"meshNoiseStatus" isEqualToString:call.method]) {
    result(_meshNoise ? [_meshNoise status] :
        @{@"supported": @YES, @"attached": @NO,
          @"processed_frames": @0, @"bypassed_frames": @0});
    return;
  }
...
- (void)dealloc {
  if (_meshNoise) [_audioManager.capturePostProcessingAdapter removeProcessing:_meshNoise];
