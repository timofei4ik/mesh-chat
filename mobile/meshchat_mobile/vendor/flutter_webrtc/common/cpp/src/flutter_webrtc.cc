...
#ifdef MESH_RNNOISE
  if (mesh_noise_ && audio_processing()) {
    static MeshNoiseBypass bypass;
    audio_processing()->SetCapturePostProcessing(&bypass);
  }
#endif
}
...
#ifdef MESH_RNNOISE
  if (method_call.method_name() == "meshNoiseConfigure") {
    if (!audio_processing() || !method_call.arguments()) {
      result->Success(EncodableValue(false));
      return;
    }
    const auto params = GetValue<EncodableMap>(*method_call.arguments());
    const bool enabled = findBoolean(params, "enabled");
    if (enabled && !mesh_noise_) {
      mesh_noise_ = std::make_unique<MeshNoiseProcessor>();
      audio_processing()->SetCapturePostProcessing(mesh_noise_.get());
    }
    if (mesh_noise_) mesh_noise_->SetEnabled(enabled);
    result->Success(EncodableValue(enabled && mesh_noise_ != nullptr));
    return;
  }
  if (method_call.method_name() == "meshNoiseStatus") {
    result->Success(EncodableValue(EncodableMap{
      {EncodableValue("supported"), EncodableValue(true)},
      {EncodableValue("attached"), EncodableValue(mesh_noise_ && mesh_noise_->enabled.load())},
      {EncodableValue("processed_frames"), EncodableValue(mesh_noise_ ? mesh_noise_->processed.load() : int64_t(0))},
      {EncodableValue("bypassed_frames"), EncodableValue(mesh_noise_ ? mesh_noise_->bypassed.load() : int64_t(0))}
    }));
    return;
  }
#endif
